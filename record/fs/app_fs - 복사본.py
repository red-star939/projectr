...
# fs_report_test로 이름 변경 반영
from src.financial_agent import utils_, conSQL, FS_to_SQL, yfinance_api, Sector, Extract_corr, fs_report_test
...
with st.sidebar:
    ...
    st.info("금융감독원 DART 및 시장 데이터 통합 분석 시스템")
...
if analyze_btn:
    with st.status(f"{target_corp} 데이터 분석 중...", expanded=True) as status:
        # 1. 데이터 수집 및 DB 적재
    ...
    if df is not None and not df.empty:
        ...
        st.download_button(
            label="💾 리포트 다운로드 (.md)",
            data=report_md,
            file_name=f"{target_corp}_분석리포트_{datetime.now().strftime('%Y%m%d')}.md",
            mime="text/markdown",
            use_container_width=True
        )
    else:
        st.error("데이터를 불러올 수 없습니다. 기업명을 다시 확인해 주세요.")

else:
    st.info("왼쪽 사이드바에서 기업을 선택하고 '정밀 분석 시작' 버튼을 눌러주세요.")