...
def extract_with_selenium(url):
    ...
    try:
        ...
        # 화면에 렌더링된 body 내부의 순수 텍스트만 긁어옵니다. (마우스 드래그 복사와 동일)
...
def process_advanced_purification():
    ...
    print("🦇 배트 컴퓨터 2중 복원(정적+동적) 프로토콜 가동 중...")
    ...
    for keyword in os.listdir(NEWS_BASE_DIR):
        ...
        print(f"\n[{keyword}] 키워드 데이터 구조 분석 및 이중 복원 시작...")
        ...
        print(f"✅ 정제 완료: {valid_count}건 유지 (2중 로직 복원 {recovered_count}건 포함) / {deleted_count}건 영구 삭제 및 DB 갱신 완료.")
