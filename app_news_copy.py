...
# ==========================================
# [설정] 메뉴 항목 동적 구성 (하드코딩 방지)
# 항목이 추가되면 아래 리스트에 딕셔너리 형태로 추가해주시면 됩니다.
# ==========================================
MENU_ITEMS = [
    {"label": "소개", "icon": "info-circle"},
    {"label": "메인", "icon": "house"},
    {"label": "세부", "icon": "gear"},
    # 예시: {"label": "대시보드", "icon": "bar-chart"},
]

MENU_LABELS = [item["label"] for item in MENU_ITEMS]
MENU_ICONS = [item["icon"] for item in MENU_ITEMS]
# 기본 선택 인덱스를 '메인'으로 동적 설정 (만약 이름이 바뀌더라도 에러 없이 0으로 세팅)
DEFAULT_PAGE_INDEX = MENU_LABELS.index("메인") if "메인" in MENU_LABELS else 0

# 확정된 Dark Neon (미래지향적) 디자인 스타일
DARK_NEON_STYLES = {
    "container": {"padding": "5!important", "background-color": "#111111", "border-radius": "10px", "border": "1px solid #333"},
    "icon": {"color": "#00ffcc", "font-size": "20px"}, 
    "nav-link": {"font-size": "16px", "text-align": "left", "margin":"5px 0px", "color": "#dddddd", "--hover-color": "#222222", "border-radius": "5px"},
    "nav-link-selected": {"background-color": "transparent", "border": "1px solid #00ffcc", "color": "#00ffcc", "box-shadow": "0 0 10px #00ffcc"},
}
...
if page == "메인":
    ...
    # 메인 페이지 이외의 다른 동적 페이지들(소개, 세부 등)은 빈 화면 리턴
    st.empty()